...
- `coros-report`：COROS 运动报告、LangGraph 工作流、RAG 训练问答、跑步视频知识导入和长期训练记忆。知识库按内容方向分类（跑鞋 / 训练），订阅的 B 站 UP 主由定时任务自动同步。
...
循环后面还有一个轻量的 Reflection 节点。它不负责重新回答，只检查草稿是否满足问题：有没有该查 COROS 却没查、该查 RAG 却没查、问官网 / 报名 / 库存 / 当前购买建议却没联网。检查不通过时，它会把缺口转成一次补救请求，让主 Agent 再走一轮工具循环；如果资料补不齐，例如缺年龄、周跑量、目标日期，它会直接追问用户。
...
- `数据`：只读展示个人数据，包括运动画像、PB、比赛照片、RAG 书籍和视频知识库、COROS FIT 归档、厨房菜谱和采购数据。知识库按「内容方向 → UP 主 → 视频」三级树展开，左侧逐级选择、右侧看内容。
- `技术`：展示项目迭代报告、RAG 流程和问题解决记录。

对话页左栏有一张**动态架构图**：回答过程中会通过 SSE 推送 `trace_step`，
对应节点和连线按顺序点亮，像把一次提问在架构图上走了一遍。
真实 Agent 调工具会点亮实际命中的模块；显式命令和 demo 模式也会走一条预期路径，
让展示时能看清「入口 → 语义路由 → 能力层 → LangGraph / MCP / RAG → LLM 生成 → 反思检查 → 回答」。
映射表在 `src/runtime/flow_map.py`，按数据来源归并而不是按函数——
每个工具一个节点的话图上会有二十几个框，反而看不清主线。

公开 Web 入口只读，写入操作仍限制在 Discord 能力频道。

## 知识库同步

`scripts/sync_bilibili.py` 把订阅的 B 站 UP 主的视频字幕同步进知识库。
订阅名单在 `data/knowledge/coros-report/sources.json`，
也可以直接发一个空间链接给 Agent（`!knowledge-source`），下一轮定时任务就会开始导入。

**限流卡在「列视频」而不是「抓字幕」**，所以节奏是围绕列表接口设计的：
视频列表缓存 20 小时，列表请求失败时退回缓存继续回填，连续出错则熔断。
熔断后写一个冷静期文件，接下来两小时的定时任务直接退出——
提高频率必须同时提高退让能力。

服务器上由 systemd 定时器每半小时跑一轮，单轮 8 条、四个来源平分：

```
agentdeck-bili-sync.timer  →  OnCalendar=*:00,30
```

单轮批量小、跑得勤，平均请求密度比原来「每天一次抓 20 条」还低，
但日上限从 20 条提到 384 条。单元文件在 `deploy/systemd/`。手动跑：
...
其中两条守的是「契约」而不是「效果」：`loop_tool_exposure` 锁住每个入口能看到哪些工具（写工具不该出现在只读入口），`flow_map_coverage` 锁住每个工具都能映射到架构图上的一个模块。后者防的是一种不报错的坏法——**忘了更新映射，图不会崩，只会说谎**。
...
`src/integrations/web_search.py` 提供 `search_web` 工具，用于查系统里没有的外部信息（赛事安排、天气、知识库外的训练问题）。**没有配置 API key 时它不会进入工具表**，模型也就不会承诺做不到的事。

```bash
# .env 里二选一
TAVILY_API_KEY=...
BRAVE_SEARCH_API_KEY=...
WEB_SEARCH_DAILY_LIMIT=50   # 每日预算，默认 50
```

三层约束：结果标为 `returns_untrusted`（读过之后本轮禁止写操作）、每日预算独立于每分钟限流（按次收费 + 公开入口无认证）、每次查询词记进日志（`evt=search_query`）。

最后一条是因为**查询词是模型生成的、会离开本机的字符串**——这是系统里唯一的外发通道，挡不住就至少要看得见。

用直连 API 而不是 Tavily 的 MCP 端点：MCP 每次调用要起一个 node 子进程（5~15 秒），而搜索是用户等着的同步调用；key 写在 MCP 的 URL 里会进进程列表和日志；`npx mcp-remote` 自动升级还导致过一次线上卡死（见迭代报告 3.44）。
